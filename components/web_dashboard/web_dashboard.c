...
static const char DASHBOARD_HTML[] =
    ...
    ".hdr{background:#1E293B;border-bottom:1px solid #334155;padding:6px 12px;display:flex;align-items:center;gap:8px;height:44px}"
    ".hdr .t{font-size:15px;font-weight:700;color:#10B981;letter-spacing:.5px}"
    ".hdr .m{font-size:13px;color:#10B981;margin-left:auto;letter-spacing:1px}"
    ".hdr .s{font-size:11px;color:#64748B}"
    ".nav{display:flex;background:#1E293B;border-bottom:1px solid #334155;overflow-x:auto;padding:0 4px;gap:2px}"
    ".nav a{color:#94A3B8;text-decoration:none;padding:10px 14px;font-size:13px;white-space:nowrap;border-bottom:2px solid transparent;cursor:pointer;min-height:44px;display:flex;align-items:center}"
    ".nav a:active,.nav a.active{color:#10B981;border-bottom-color:#10B981;background:rgba(16,185,129,.06)}"
    ".main{padding:12px;max-width:1200px;margin:0 auto;display:flex;flex-direction:column;gap:8px}"
    ...
    ".card h3{font-size:11px;color:#64748B;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px}"
    ...
    "td{color:#CBD5E1;font-family:'SF Mono','Fira Code','Courier New',monospace;font-size:12px}"
    "tr:hover{background:rgba(16,185,129,.04)}"
    ".badge{display:inline-block;padding:2px 8px;font-size:10px;font-weight:600;text-transform:uppercase;border-radius:4px}"
    ".badge.open{background:rgba(16,185,129,.15);color:#10B981}"
    ".badge.wpa2{background:rgba(245,158,11,.15);color:#F59E0B}"
    ".badge.wpa3{background:rgba(139,92,246,.15);color:#8B5CF6}"
    ".btn{display:inline-block;min-height:44px;padding:8px 18px;border:none;border-radius:6px;font-size:13px;font-weight:600;cursor:pointer;margin:2px;font-family:inherit}"
    ...
    ".btn-sm{min-height:36px;padding:4px 14px;font-size:12px}"
    ".form-group{margin-bottom:12px}"
    ".form-group label{display:block;font-size:12px;color:#64748B;margin-bottom:4px}"
    ".form-group input,.form-group select{width:100%;min-height:44px;padding:8px 12px;background:#0F172A;border:1px solid #334155;border-radius:6px;color:#F1F5F9;font-size:14px;font-family:inherit}"
    ".form-group input:focus,.form-group select:focus{outline:none;border-color:#10B981}"
    ".log-viewer{background:#0F172A;border:1px solid #334155;border-radius:8px;padding:12px;font-family:'SF Mono','Fira Code','Courier New',monospace;font-size:11px;height:300px;overflow-y:auto;white-space:pre-wrap;line-height:1.6}"
    ".log-viewer .info{color:#64748B}"
    ".log-viewer .warn{color:#F59E0B}"
    ".log-viewer .error{color:#EF4444}"
    ".log-viewer .cred{color:#10B981;font-weight:700}"
    ".empty{text-align:center;padding:40px;color:#475569;font-size:14px}"
    ".capture-entry{background:#1E293B;border:1px solid #334155;border-radius:6px;padding:12px;margin-bottom:8px;font-size:12px}"
    ".capture-entry .src{color:#3B82F6;font-weight:600}"
    ".capture-entry .ts{color:#64748B;float:right}"
    ".capture-entry .user{color:#10B981}"
    ".capture-entry .pass{color:#EF4444}"
    ...
    ".modal-content{background:#1E293B;border:1px solid #334155;border-radius:12px;padding:24px;max-width:480px;width:90%;max-height:80vh;overflow-y:auto}"
    ".modal-content h2{color:#10B981;margin-bottom:16px}"
    ".step-badge{padding:6px 14px;font-size:11px;font-weight:600;background:#1E293B;color:#64748B;border:1px solid #334155;border-radius:20px;display:inline-block}"
    ".step-badge.active{background:rgba(245,158,11,.15);color:#F59E0B;border-color:#F59E0B}"
    ".step-badge.done{background:rgba(16,185,129,.15);color:#10B981;border-color:#10B981}"
    ".step-badge.cracked{background:rgba(239,68,68,.15);color:#EF4444;border-color:#EF4444}"
    "@keyframes stepPulse{0%,100%{opacity:1}50%{opacity:.6}}"
    ".bar{display:none;position:fixed;bottom:0;left:0;width:100%;height:56px;background:#1E293B;border-top:1px solid #334155;z-index:100}"
    ".bar a{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#64748B;text-decoration:none;font-size:9px;min-height:44px;border:none;background:none;cursor:pointer}"
    ".bar a:active,.bar a.active{color:#10B981}"
    ".bar a .i{font-size:18px;line-height:1.2}"
    ".mo{display:none;position:fixed;bottom:56px;left:0;width:100%;max-height:50vh;background:#1E293B;border:1px solid #334155;border-bottom:none;z-index:99;overflow-y:auto}"
    ".mo.active{display:block}"
    ".mo a{display:flex;align-items:center;gap:10px;padding:12px 16px;color:#CBD5E1;text-decoration:none;border-bottom:1px solid #334155;font-size:14px;min-height:44px}"
    ".mo a:active{color:#10B981;background:rgba(16,185,129,.06)}"
    ".mo a .i{font-size:18px}"
    "@media(max-width:480px){"
    ".nav{display:none}"
    ".bar{display:flex}"
    ".main{padding-bottom:68px;padding:8px}"
    ".hdr .t{font-size:13px}"
    ".hdr .m{font-size:12px}"
    ".card .value{font-size:20px}"
    ".cards{grid-template-columns:1fr 1fr}"
    "}"
    "</style></head><body>"
    "<div class='hdr'>"
    "<div class='t'>VIPER-S3</div>"
    ...
    "<a href='#' data-panel='autopwn' onclick='return switchPanel(\"autopwn\")' style='color:#F59E0B;font-weight:700'>&#x26A1; AUTO-PWNER</a>"
    "<a href='#' data-panel='map' onclick='return switchPanel(\"map\")'>&#x1F5FA; Map</a>"
    "<a href='#' onclick='window.open(\"/api/report?key=\"+(localStorage.getItem(\"viper_key\")||\"viper\"));return false' style='color:#F59E0B'>&#x1F4CB; Report</a>"
    "</div>"
    ...
    "</div></div>"
    "<div class='bar'>"
    "<a href='#' class='active' data-panel='overview' onclick='return switchPanel(\"overview\")'><span class='i'>&#x25CF;</span>Home</a>"
    "<a href='#' data-panel='attacks' onclick='return switchPanel(\"attacks\")'><span class='i'>&#x2694;</span>Attacks</a>"
    "<a href='#' data-panel='captures' onclick='return switchPanel(\"captures\")'><span class='i'>&#x25A3;</span>Captures</a>"
    "<a href='#' data-panel='logs' onclick='return switchPanel(\"logs\")'><span class='i'>&#x2630;</span>Logs</a>"
    "<a href='#' data-panel='autopwn' onclick='return switchPanel(\"autopwn\")'><span class='i'>&#x26A1;</span>Auto</a>"
    "<a href='#' onclick='return toggleMore()'><span class='i'>&#x2630;</span>More</a>"
    "</div>"
