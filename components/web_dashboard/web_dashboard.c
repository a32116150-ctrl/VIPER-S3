...
static const char DASHBOARD_HTML[] =
    ...
    "body{font-family:'Courier New',monospace;background:#F2EFE9;color:#1A1A2E;min-height:100vh;display:flex;flex-direction:column}"
    ".hdr{background:#1A1A2E;border-bottom:3px solid #1B7A3D;padding:2px 8px;display:flex;align-items:center;gap:4px;height:40px;flex-shrink:0}"
    ".hdr .t{font-size:13px;color:#1B7A3D;letter-spacing:2px;text-transform:uppercase;white-space:nowrap}"
    ".hdr .m{font-size:13px;color:#1B7A3D;letter-spacing:2px;margin-left:auto}"
    ".hdr .s{font-size:10px;color:#6B7280;white-space:nowrap}"
    ".lo{display:flex;flex:1;min-height:0}"
    ".nav{width:56px;background:#1A1A2E;border-right:2px solid #2D2D3E;display:flex;flex-direction:column;align-items:center;padding:4px 0;overflow-y:auto;flex-shrink:0}"
    ".nav a{display:flex;flex-direction:column;align-items:center;justify-content:center;width:48px;min-height:44px;color:#6B7280;text-decoration:none;font-size:8px;border:1px solid transparent;cursor:pointer}"
    ".nav a:active,.nav a.active{color:#1B7A3D;border-color:#1B7A3D;background:rgba(27,122,61,.08)}"
    ".nav a .i{font-size:14px;line-height:1.2}"
    ".main{flex:1;padding:4px;overflow-y:auto;display:flex;flex-direction:column;gap:4px}"
    ".panel{display:none;width:100%}"
    ".panel.active{display:flex;flex-direction:column;gap:4px}"
    ".cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:6px;margin-bottom:4px}"
    ".card{background:#FAF8F4;border:2px solid #2D2D3E;padding:6px;width:100%}"
    ".card h3{font-size:10px;color:#6B7280;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px}"
    ".card .value{font-size:18px;color:#1A1A2E}"
    ".card .value.green{color:#1B7A3D}"
    ".card .value.red{color:#C62828}"
    ".card .value.blue{color:#2563EB}"
    ".card .value.yellow{color:#B8860B}"
    "table{width:100%;border-collapse:collapse;font-size:11px}"
    "th,td{padding:4px 8px;text-align:left;border:1px solid #2D2D3E}"
    "th{color:#6B7280;font-size:9px;text-transform:uppercase;letter-spacing:1px;background:#E8E4D9}"
    "td{color:#1A1A2E;font-family:'Courier New',monospace;font-size:11px}"
    "tr:hover{background:rgba(27,122,61,.04)}"
    ".badge{display:inline-block;padding:1px 6px;font-size:9px;text-transform:uppercase;border:1px solid currentColor}"
    ".badge.open{border-color:#1B7A3D;color:#1B7A3D}"
    ".badge.wpa2{border-color:#B8860B;color:#B8860B}"
    ".badge.wpa3{border-color:#7C3AED;color:#7C3AED}"
    ".btn{display:inline-block;min-height:44px;padding:8px 16px;border:2px solid #2D2D3E;background:#FAF8F4;color:#1A1A2E;font-family:'Courier New',monospace;font-size:14px;cursor:pointer;margin:2px}"
    ".btn:active{background:rgba(27,122,61,.1);border-color:#1B7A3D}"
    ".btn-primary{border-color:#1B7A3D;color:#1B7A3D}"
    ".btn-danger{border-color:#C62828;color:#C62828}"
    ".btn-warn{border-color:#B8860B;color:#B8860B}"
    ".btn-sm{min-height:36px;padding:4px 12px;font-size:12px}"
    ".form-group{margin-bottom:8px}"
    ".form-group label{display:block;font-size:11px;color:#6B7280;margin-bottom:2px;text-transform:uppercase;letter-spacing:1px}"
    ".form-group input,.form-group select{width:100%;min-height:44px;padding:8px;background:#FFFFFF;border:2px solid #2D2D3E;color:#1A1A2E;font-family:'Courier New',monospace;font-size:14px}"
    ".form-group input:focus,.form-group select:focus{outline:none;border-color:#1B7A3D}"
    ".log-viewer{background:#E8E4D9;border:2px solid #2D2D3E;padding:8px;font-family:'Courier New',monospace;font-size:11px;height:180px;overflow-y:auto;white-space:pre-wrap;line-height:1.5}"
    ".log-viewer .info{color:#6B7280}"
    ".log-viewer .warn{color:#B8860B}"
    ".log-viewer .error{color:#C62828}"
    ".log-viewer .cred{color:#1B7A3D}"
    ".empty{text-align:center;padding:20px;color:#9CA3AF;font-size:12px}"
    ".capture-entry{background:#FAF8F4;border:2px solid #2D2D3E;padding:8px;margin-bottom:4px;font-size:11px}"
    ".capture-entry .src{color:#2563EB}"
    ".capture-entry .ts{color:#6B7280;float:right}"
    ".capture-entry .user{color:#1B7A3D}"
    ".capture-entry .pass{color:#C62828}"
    ".modal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(26,26,46,.7);z-index:1000;justify-content:center;align-items:center}"
    ".modal.active{display:flex}"
    ".modal-content{background:#FAF8F4;border:2px solid #1B7A3D;padding:16px;max-width:480px;width:90%;max-height:80vh;overflow-y:auto}"
    ".modal-content h2{color:#1B7A3D;margin-bottom:12px}"
    ".step-badge{display:inline-block;padding:4px 10px;font-size:10px;background:#FAF8F4;color:#6B7280;border:2px solid #2D2D3E}"
    ".step-badge.active{color:#B8860B;border-color:#B8860B}"
    ".step-badge.done{color:#1B7A3D;border-color:#1B7A3D}"
    ".step-badge.cracked{color:#C62828;border-color:#C62828}"
    "@keyframes stepPulse{0%,100%{opacity:1}50%{opacity:.6}}"
    ".bar{display:none;position:fixed;bottom:0;left:0;width:100%;height:52px;background:#1A1A2E;border-top:2px solid #2D2D3E;z-index:100}"
    ".bar a{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#6B7280;text-decoration:none;font-size:8px;min-height:44px;border:none;background:none;cursor:pointer}"
    ".bar a:active,.bar a.active{color:#1B7A3D}"
    ".bar a .i{font-size:16px;line-height:1.2}"
    ".mo{display:none;position:fixed;bottom:52px;left:0;width:100%;max-height:50vh;background:#FAF8F4;border:2px solid #2D2D3E;border-bottom:none;z-index:99;overflow-y:auto}"
    ".mo.active{display:block}"
    ".mo a{display:flex;align-items:center;gap:8px;padding:10px 16px;color:#1A1A2E;text-decoration:none;border-bottom:1px solid #2D2D3E;font-size:13px;min-height:44px}"
    ".mo a:active{color:#1B7A3D;background:rgba(27,122,61,.04)}"
    ".mo a .i{font-size:16px}"
    "@media(max-width:480px){"
    ".nav{display:none}"
    ".bar{display:flex}"
    ".main{padding-bottom:60px}"
    ".hdr .t{font-size:11px}"
    ".hdr .m{font-size:11px}"
    ".card .value{font-size:16px}"
    ".cards{grid-template-columns:1fr 1fr}"
    "}"
    "</style></head><body>"
    "<div class='hdr'>"
    "<div class='t'>CYBER-VIPER</div>"
    "<div class='m' id='mascot'>[ - _ - ]</div>"
    "<div class='s'><span id='heapText'>0</span> KB</div>"
    "</div>"
    "<div class='lo'>"
    "<div class='nav'>"
    "<a href='#' class='active' data-panel='overview' onclick='return switchPanel(\"overview\")'><span class='i'>&#x25CF;</span>OV</a>"
    "<a href='#' data-panel='wifi' onclick='return switchPanel(\"wifi\")'><span class='i'>&#x25CB;</span>WF</a>"
    "<a href='#' data-panel='attacks' onclick='return switchPanel(\"attacks\")'><span class='i'>&#x2694;</span>AT</a>"
    "<a href='#' data-panel='captures' onclick='return switchPanel(\"captures\")'><span class='i'>&#x25A3;</span>CP</a>"
    "<a href='#' data-panel='camera' onclick='return switchPanel(\"camera\")'><span class='i'>&#x25A0;</span>CM</a>"
    "<a href='#' data-panel='logs' onclick='return switchPanel(\"logs\")'><span class='i'>&#x2630;</span>LG</a>"
    "<a href='#' data-panel='ble' onclick='return switchPanel(\"ble\")'><span class='i'>&#x25A1;</span>BL</a>"
    "<a href='#' data-panel='config' onclick='return switchPanel(\"config\")'><span class='i'>&#x2699;</span>CF</a>"
    "<a href='#' data-panel='responder' onclick='return switchPanel(\"responder\")'><span class='i'>&#x25B7;</span>RS</a>"
    "<a href='#' data-panel='crack' onclick='return switchPanel(\"crack\")'><span class='i'>&#x25C6;</span>CK</a>"
    "<a href='#' data-panel='ir' onclick='return switchPanel(\"ir\")'><span class='i'>&#x25B2;</span>IR</a>"
    "<a href='#' data-panel='nfc' onclick='return switchPanel(\"nfc\")'><span class='i'>&#x25B4;</span>NF</a>"
    "<a href='#' data-panel='protocol' onclick='return switchPanel(\"protocol\")'><span class='i'>&#x25BC;</span>PR</a>"
    "<a href='#' data-panel='usb' onclick='return switchPanel(\"usb\")'><span class='i'>&#x25A8;</span>UB</a>"
    "<a href='#' data-panel='ai' onclick='return switchPanel(\"ai\")'><span class='i'>&#x25CB;</span>AI</a>"
    "<a href='#' data-panel='orchestrator' onclick='return switchPanel(\"orchestrator\")'><span class='i'>&#x2699;</span>OR</a>"
    "<a href='#' data-panel='autopwn' onclick='return switchPanel(\"autopwn\")' style='color:#FF003C'><span class='i'>&#x26A1;</span>AP</a>"
    "<a href='#' data-panel='map' onclick='return switchPanel(\"map\")'><span class='i'>&#x25B7;</span>MP</a>"
    "<a href='#' onclick='window.open(\"/api/report?key=\"+(localStorage.getItem(\"viper_key\")||\"viper\"));return false'><span class='i'>&#x25A1;</span>RP</a>"
    "</div>"
    ...
    "<a href='#' data-panel='captures' onclick='return switchPanel(\"captures\")'><span class='i'>&#x25A3;</span>CP</a>"
    "<a href='#' data-panel='logs' onclick='return switchPanel(\"logs\")'><span class='i'>&#x2630;</span>LG</a>"
    "<a href='#' data-panel='autopwn' onclick='return switchPanel(\"autopwn\")'><span class='i'>&#x26A1;</span>AP</a>"
    "<a href='#' onclick='return toggleMore()'><span class='i'>&#x2630;</span>MO</a>"
    "</div>"
    "<div class='mo' id='moreOverlay'>"
    "<a href='#' data-panel='wifi' onclick='switchPanel(\"wifi\");toggleMore();return false'><span class='i'>&#x25CB;</span>WiFi</a>"
    "<a href='#' data-panel='camera' onclick='switchPanel(\"camera\");toggleMore();return false'><span class='i'>&#x25A0;</span>Camera</a>"
    "<a href='#' data-panel='ble' onclick='switchPanel(\"ble\");toggleMore();return false'><span class='i'>&#x25A1;</span>BLE</a>"
    "<a href='#' data-panel='config' onclick='switchPanel(\"config\");toggleMore();return false'><span class='i'>&#x2699;</span>Config</a>"
    "<a href='#' data-panel='responder' onclick='switchPanel(\"responder\");toggleMore();return false'><span class='i'>&#x25B7;</span>Responder</a>"
    "<a href='#' data-panel='crack' onclick='switchPanel(\"crack\");toggleMore();return false'><span class='i'>&#x25C6;</span>Crack</a>"
    "<a href='#' data-panel='ir' onclick='switchPanel(\"ir\");toggleMore();return false'><span class='i'>&#x25B2;</span>IR</a>"
    "<a href='#' data-panel='nfc' onclick='switchPanel(\"nfc\");toggleMore();return false'><span class='i'>&#x25B4;</span>NFC</a>"
    "<a href='#' data-panel='protocol' onclick='switchPanel(\"protocol\");toggleMore();return false'><span class='i'>&#x25BC;</span>Protocol</a>"
    "<a href='#' data-panel='usb' onclick='switchPanel(\"usb\");toggleMore();return false'><span class='i'>&#x25A8;</span>USB</a>"
    "<a href='#' data-panel='ai' onclick='switchPanel(\"ai\");toggleMore();return false'><span class='i'>&#x25CB;</span>AI</a>"
    "<a href='#' data-panel='orchestrator' onclick='switchPanel(\"orchestrator\");toggleMore();return false'><span class='i'>&#x2699;</span>Orch</a>"
    "<a href='#' data-panel='map' onclick='switchPanel(\"map\");toggleMore();return false'><span class='i'>&#x25B7;</span>Map</a>"
    "<a href='#' onclick='window.open(\"/api/report?key=\"+(localStorage.getItem(\"viper_key\")||\"viper\"));toggleMore();return false'><span class='i'>&#x25A1;</span>Report</a>"
    "</div>"
    ...
    "if(s=='scan'){el.textContent='[ o _ o ]'}"
    "else if(s=='attack'){el.textContent='[ > _ < ]'}"
    "else if(s=='crack'){el.textContent='[ $_$ ]'}"
    "else if(s=='dead'){el.textContent='[ x _ x ]'}"
    "else{el.textContent='[ - _ - ]'}"
    "}"
    "function toggleMore(){"
    "var el=document.getElementById('moreOverlay');if(!el)return false;"
    "el.classList.toggle('active');return false"
    "}"
    "function closeMore(){"
    "var el=document.getElementById('moreOverlay');if(el)el.classList.remove('active')"
    "}"
    "function switchPanel(name){"
    "console.log('Switch panel: '+name);"
    "try{"
    "closeMore();"
    "document.querySelectorAll('.nav a, .bar a').forEach(function(x){x.classList.remove('active')});"
    "var el=document.querySelector('.nav a[data-panel=\"'+name+'\"],.bar a[data-panel=\"'+name+'\"]');if(el)el.classList.add('active');"
    ...
    "if(name=='overview'){refreshOverview();updateMascot('idle')}"
    "if(name=='wifi')updateMascot('scan');"
    "if(name=='attacks')updateMascot('attack');"
    "if(name=='ble'){refreshProprietary();refreshMitmStatus();updateMascot('scan')}"
    "if(name=='config'){loadConfig();updateMascot('idle')}"
    "if(name=='responder'){refreshResponderStatus();updateMascot('idle')}"
    "if(name=='crack'){refreshCrackStatus();updateMascot('crack')}"
    "if(name=='protocol'){refreshCanaryList();refreshBehavioral();updateMascot('idle')}"
    "if(name=='ir')updateMascot('scan');"
    "if(name=='nfc')updateMascot('scan');"
    "if(name=='usb'){refreshUsbStatus();updateMascot('idle')}"
    "if(name=='ai'){refreshTwins();refreshModels();updateMascot('idle')}"
    "if(name=='orchestrator'){refreshHealth();updateMascot('idle')}"
    ...
    "return false"
    "}"
    "function scanWifi(){updateMascot('scan');"
...
"}"
    ...
    "function startAutoPwner(){updateMascot('attack');"
